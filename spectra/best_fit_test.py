...
f = spt.get_spec_fit(lib, obs_wl, obs_W, k = .2)
print("Done.")
print(np.min(np.absolute(f-1)))
best_fit = lib[np.argmin(np.absolute(f-1))]
print(best_fit)

#best_fit = "p5750:g+4.0:m0.0:t01:z+0.00:a+0.00.GES4750.fits"
...
data2 = spt.limit_spec(synt_data, l1, l2)
rbest_data = spt.limit_spec(best_data, l1, l2)
robs_data = spt.limit_spec(obs_data, l1, l2)
plt.plot(robs_data[0], robs_data[1]/np.mean(robs_data[1]))
