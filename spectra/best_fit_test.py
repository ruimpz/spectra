...
def get_spec_fit(library, wl_obs, W_obs, \
    ...
    for i, synt_spec in enumerate(library):
        ...
        wl_calculated, W_synt, inds = spt.get_line_Ws(synt_data, wl_obs, get_inds=True, count=True, k = k, limit=True)
...
f = get_spec_fit(lib, obs_wl, obs_W, k = .2)
print("Done.")
...
plt.plot(robs_data[0], robs_data[1]-.4)
